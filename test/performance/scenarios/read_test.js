...
export default function () {
    group('Read Only API Tests', function () {
        ...
        if (movieListRes.status === 200) {
            const movieIds = movieListRes.json()['data']['movies']?.map(movie => movie['id']);
            if (movieIds.length > 0) {
                const randomMovieId = movieIds[Math.floor(Math.random() * movieIds.length)];
                const showtimeQueryParams = generateShowtimeQueryParams(randomMovieId);
                const showtimeRes = http.get(
                    `${BASE_URL}/api/v1/showtimes${showtimeQueryParams}`,
                    { headers: authHeaders }
                );
                checkResponse(showtimeRes, 'Get showtimes list');
            }
        }
    })
}

function generateShowtimeQueryParams(movieId) {
    const params = new URLSearchParams();
    params.set('movie_id', movieId.toString());
    return params.toString();
}