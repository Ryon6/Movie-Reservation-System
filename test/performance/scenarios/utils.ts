...
export function login(username: string, password: string) {
    ...
    const success = check(loginRes, {
        ...
        'has token': (r) => {
            try {
                const body = r.body ? JSON.parse(r.body as string) : null;
                return body?.token;
            } catch (e) {
                return false;
            }
    ...
    try {
        const body = loginRes.body ? JSON.parse(loginRes.body as string) : null;
...
// 检查响应状态
export function checkResponse(res: http.Response, checkName: string) {
