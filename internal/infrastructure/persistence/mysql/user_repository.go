...
func (r *gormUserRepository) Create(ctx context.Context, user *user.User) error {
	...
	logger.Info("user created successfully", applog.Uint("user_id", user.ID))
	return nil
}
...
func (r *gormUserRepository) FindByID(ctx context.Context, id uint) (*user.User, error) {
	...
	logger.Info("user found by ID", applog.Uint("ID", user.ID))
	return &user, nil
}
...
func (r *gormUserRepository) FindByUsername(ctx context.Context, username string) (*user.User, error) {
	...
	logger.Info("user found by username", applog.String("username", user.Username))
	return &user, nil
}
...
func (r *gormUserRepository) FindByEmail(ctx context.Context, email string) (*user.User, error) {
	...
	logger.Info("user found by email", applog.String("email", user.Email))
	return &user, nil
}
...
func (r *gormUserRepository) Update(ctx context.Context, user *user.User) error {
	...
	if err := r.db.WithContext(ctx).Save(user).Error; err != nil {
		logger.Error("failed to update user", applog.Uint("user_id", user.ID), applog.Error(err))
		return err
	}
	logger.Info("user updated successfully", applog.Uint("user_id", user.ID))
	return nil
}
