...
type ShowtimeRepository interface {
	...
	List(ctx context.Context, page, pageSize int, filters map[string]interface{}) ([]*Showtime, error)
	Update(ctx context.Context, showtime *Showtime) error
	Delete(ctx context.Context, id uint) error

	// 检查指定时间段内是否存在与给定影厅冲突的场次（排除指定ID的场次）
	CheckOverlap(ctx context.Context, hallID uint, startDate, endDate time.Time, excludeShowtimeID ...uint) (bool, error)
	// 查询指定电影在日期范围内的所有场次
